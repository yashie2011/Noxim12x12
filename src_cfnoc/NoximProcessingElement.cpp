...
int NoximProcessingElement::get_slice(NoximCoord src_coord, NoximCoord dest_coord){
	...
	if (!is_mc(src_coord)){
		...
		if(src_coord.y == 8 ){return 8;}  // column 4 choose slice 8
		if(src_coord.y == 9 ){return 9;}  // column 5 choose slice 9
		if(src_coord.y == 10 ){return 10;}  // column 6 choose slice 10
		if(src_coord.y == 11 ){return 11;}  // column 7 choose slice 11

	}
	else return 12;    // Mem Controller chooses slice 8
...
void NoximProcessingElement::push_packet(){
	...
	if (true) {
		...
		int mem_c = packet.dst_id/13;  // This is a hack to find out the memory controller number which is used to reserve the token. should change it
...
NoximFlit NoximProcessingElement::nextFlit(int slice)
{
	...
	if(!is_mc(id2Coord(local_id))){
		int mem_c = packet.dst_id /13; // Another bloody hack.
