...
#define DIRECTIONS             4
#define SLICES				   13      // Added this for base work implementation
...
enum mem_controllers {m1 = 0, m2 = 13, m3 = 26, m4 = 39, m5 = 52, m6 = 65, m7 = 78, m8 = 91,
					  m9 = 104, m10 = 117, m11 = 130, m12 = 143};
...
inline bool is_mc(NoximCoord src_coord)
{
	if( coord2Id(src_coord) == m1 || coord2Id(src_coord) == m2 || coord2Id(src_coord) == m3
			...
			|| coord2Id(src_coord) == m7 || coord2Id(src_coord) == m8 || coord2Id(src_coord) == m9
			|| coord2Id(src_coord) == m10 || coord2Id(src_coord) == m11 || coord2Id(src_coord) == m12)
				return true;
	else
		return false;
}
...
inline bool is_mc(int local_id)
{
	if( local_id == m1 || local_id == m2 || local_id == m3
			...
			|| local_id == m7 || local_id == m8 || local_id == m9 || local_id == m10
			|| local_id == m11 || local_id == m12)
				return true;
	else
		return false;
}
